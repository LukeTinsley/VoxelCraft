...
public class BlockV3
{
    ...
    public BlockV3(BlockType b, Vector3 pos, GameObject p, ChunkV3 chunk)
    {
        blockType = b;
        owner = chunk;
        parent = p;
    ...
    {
        if (i == -1)
            i = WorldV1.chunkSize - 1;
        else if (i == WorldV1.chunkSize)
            i = 0;
        return i;
    }
    ...
    public bool HasSolidNeighbour(int x, int y, int z)
    {
        ...
        if (x < 0 || x >= WorldV1.chunkSize ||
            y < 0 || y >= WorldV1.chunkSize ||
            z < 0 || z >= WorldV1.chunkSize)
        {  //block in a neighbouring chunk

            Vector3 neighbourChunkPos = this.parent.transform.position +
                                        new Vector3((x - (int)position.x) * WorldV1.chunkSize,
                                            (y - (int)position.y) * WorldV1.chunkSize,
                                            (z - (int)position.z) * WorldV1.chunkSize);
            string nName = WorldV1.BuildChunkName(neighbourChunkPos);
            ...
            ChunkV3 nChunk;
            if (WorldV1.chunks.TryGetValue(nName, out nChunk))
            {
