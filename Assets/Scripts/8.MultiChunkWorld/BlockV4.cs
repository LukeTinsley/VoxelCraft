...
public class BlockV4
{
    ...
    public BlockV4(BlockType b, Vector3 pos, GameObject p, ChunkV4 chunk)
    {
        blockType = b;
        owner = chunk;
        parent = p;
    ...
    {
        if (i == -1)
            i = WorldV2.chunkSize - 1;
        else if (i == WorldV2.chunkSize)
            i = 0;
        return i;
    }
    ...
    public bool HasSolidNeighbour(int x, int y, int z)
    {
        ...
        if (x < 0 || x >= WorldV2.chunkSize ||
            y < 0 || y >= WorldV2.chunkSize ||
            z < 0 || z >= WorldV2.chunkSize)
        {  //block in a neighbouring chunk

            Vector3 neighbourChunkPos = this.parent.transform.position +
                                        new Vector3((x - (int)position.x) * WorldV2.chunkSize,
                                            (y - (int)position.y) * WorldV2.chunkSize,
                                            (z - (int)position.z) * WorldV2.chunkSize);
            string nName = WorldV2.BuildChunkName(neighbourChunkPos);
            ...
            ChunkV4 nChunk;
            if (WorldV2.chunks.TryGetValue(nName, out nChunk))
            {
